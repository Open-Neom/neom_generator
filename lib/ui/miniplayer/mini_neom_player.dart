...
class _PulsingDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return VisualAnimation(
      ...
      builder: (_, clock, _) => AnimatedBuilder(
        animation: clock,
        builder: (_, _) {
          final pulse = 1 - (2 * clock.value - 1).abs();
          return Container(
            width: 8,
            height: 8,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color: color.withAlpha((120 + (pulse * 135)).round()),
              boxShadow: [
                BoxShadow(
                  color: color.withAlpha((pulse * 80).round()),
                  blurRadius: 6,
                ),
              ],
            ),
...
class _MiniWaveform extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return VisualAnimation(
      ...
      builder: (_, clock, _) => RepaintBoundary(
        child: CustomPaint(
          painter: _SineWavePainter(clock: clock, color: color),
        ),
      ),
    );
  }
...
class _SineWavePainter extends CustomPainter {
  ...
  double get phase => clock.value * 2 * pi;
  final Color color;
  _SineWavePainter({required this.clock, required this.color})
    : super(repaint: clock);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= size.width; x += 1) {
      final normalized = x / size.width;
      final y =
          midY + sin(normalized * 4 * 3.14159 + phase + 0.8) * amplitude * 0.7;
      if (x == 0) {
        path2.moveTo(x, y);
      } else {
        path2.lineTo(x, y);
